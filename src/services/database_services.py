...
class DatabaseService:
    ...
    async def update_verification_status(
        self, verification_id: int, status: VerificationStatusEnum
    ...
    async def get_high_risk_verifications(
    ...
    async def create_report(self, **kwargs) -> ReportRecord:
        ...
        kwargs["fir_number"] = (
            f"FIR-{datetime.now(timezone.utc).year}-{uuid.uuid4().hex[:8].upper()}"
        )
    ...
    async def update_report_status(
        self, report_id: int, status: ReportStatusEnum
    ) -> ReportRecord | None:
    ...
    # ---------- HEATMAP OPERATIONS ----------
    async def add_heatmap_point(
        self, lat: float, lng: float, risk_score: float, fraud_category: str
    ):
        heatmap = HeatmapData(
            location_lat=lat,
            location_lng=lng,
    ...
    async def get_dashboard_stats(self) -> dict[str, Any]:
        ...
        active_reports = await self.session.execute(
            select(func.count(ReportRecord.id)).where(
                ReportRecord.status != ReportStatusEnum.CLOSED
            )
        )

        # Average risk score
        avg_risk = await self.session.execute(
            select(func.avg(VerificationRecord.risk_score)).where(
                func.date(VerificationRecord.created_at) == today
            )
        )
